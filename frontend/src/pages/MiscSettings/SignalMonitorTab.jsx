...
import { Badge, Button, Card, Descriptions, Divider, Space, Spin, Alert } from "antd";
...
const getStatus = (type, info) => {
  if (type === "amr") return info ? "success" : "default";
};

export default function SignalMonitorTab() {
  ...
  const reconnectM = useMutation({
    mutationFn: async () => {
      const { type, key } = selected;
      await fetch(`${API}/api/health/signals/${type}/${key}/reconnect`, { method: "POST" });
    },
    onSuccess: () => {
      refetch();
      detail.refetch();
  ...
  const renderBadges = (items, type) => {
    if (!items || typeof items !== "object") return <span style={{ opacity: 0.7 }}>데이터 없음</span>;
    return (
  ...
  return (
    <Card title="신호 상태 모니터" bordered>
      ...
      {error && <Alert type="error" message="신호 로드 실패" description={error.message} />}
      {data && (
        <Space direction="vertical" size="large" style={{ width: "100%" }}>
          <Descriptions bordered size="small" column={1} labelStyle={{ width: 120 }}>
            <Descriptions.Item label="AMR">{renderBadges(data.amr, "amr")}</Descriptions.Item>
          </Descriptions>

          <Card title="상세 정보" size="small">
            {!selected.type && <Alert message="항목을 클릭하세요" type="info" />}
            {detail.isLoading && <Spin tip="조회 중..." />}
            {detail.error && (
              <Alert type="error" message="상세 조회 실패" description={detail.error.message} />
            )}
            {detail.data && (
              <pre style={{ margin: 0 }}>{JSON.stringify(detail.data, null, 2)}</pre>
            )}
          </Card>
        </Space>
      )}
