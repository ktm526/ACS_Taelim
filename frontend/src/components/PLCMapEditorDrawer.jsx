...
export default function PLCMapEditorDrawer({ open, onClose, apiBase = "", stations = [] }) {
  ...
  const stationAutoOptions = useMemo(() => {
    return (stations || [])
      .map((s) => {
        ...
        const value = name; // PLCMap.amr_station은 station name을 저장
        return {
          value,
          label: `${name} (#${id})`,
        };
    ...
    <>
      ...
      >
        <Table
          ...
          columns={[
            { title: "PLC Bit", dataIndex: "id", width: 90 },
            { title: "AMR Station", dataIndex: "amr_station", width: 160, ellipsis: true },
            { title: "Mani ID", dataIndex: "mani_id", width: 120, ellipsis: true },
            { title: "Class", dataIndex: "class", width: 100 },
            { title: "Product", dataIndex: "product_type", width: 90 },
      ...
      <Modal
        title={editing ? `PLCMap 수정 (PLC Bit ${editing.id})` : "PLCMap 추가"}
        open={editOpen}
      ...
      >
        <Form form={form} layout="vertical">
          <Form.Item
            name="id"
            label="PLC Bit(ID)"
            rules={[{ required: true, message: "PLC Bit를 입력하세요" }]}
          >
          ...
          <Form.Item
            name="amr_station"
            label="AMR Station"
            rules={[{ required: true, message: "AMR Station을 입력/선택하세요" }]}
          >
            <AutoComplete
          ...
          <Form.Item
            name="class"
            label="Class"
            rules={[{ required: true, message: "class를 선택하세요" }]}
          >
